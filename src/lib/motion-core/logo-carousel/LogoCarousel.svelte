<script lang="ts">
	...
	interface Props {
		/** Preserve supplied order for grouped slides when false. */
		shuffle?: boolean;
		/** Local accessibility extension: suspend cycling without replacing logos. */
		paused?: boolean;
		/**
	...
	let logoSets = $derived(distributeLogos(logos, columnCount, isMounted && shuffle));
</script>

<div class={cn("flex space-x-4", className)}>
	{#each logoSets as logos, index (index)}
		<LogoColumn {logos} {index} {cycleInterval} {paused} />
	{/each}
</div>
