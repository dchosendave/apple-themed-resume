<script lang="ts">
	...
	const fragmentShader = `
		...
		void main() {
			vec2 baseUv = uFocus + (getCoverUV(vUv, uOriginalTextureSize) - uFocus) / max(uZoom, 1.0);
			vec2 depthUv = uFocus + (getCoverUV(vUv, uDepthTextureSize) - uFocus) / max(uZoom, 1.0);
	...
	$effect(() => {
		...
		uniforms.uFocus.value.set(focusX, focusY);
	});
	...
	const setupScene = (targetCanvas: HTMLCanvasElement) => {
		...
		const focusUniform = new Vec2(focusX, focusY);
