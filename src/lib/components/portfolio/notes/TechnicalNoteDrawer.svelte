    ...
    <Dialog.Root open={true} onOpenChange={handleOpenChange}>
        ...
        >
            <div class="flex max-h-[88dvh] flex-col">
                <Dialog.Header
                    class="flex-row items-start justify-between gap-4 px-6 pb-4 pt-6 sm:px-7"
                >
                    <div class="min-w-0 space-y-3">
                        <p class="apple-section-title mb-0">Technical Memo</p>
                        <div class="flex flex-wrap items-center gap-2">
                            <span class="apple-badge">{note.category}</span>
                            <span class="apple-chip text-[0.68rem]">
                                {note.readTime}
                            </span>
                        </div>

                        <Dialog.Title
                            class="text-xl font-semibold leading-tight tracking-normal [color:var(--ios-text-primary)]"
                        >
                            {note.title}
                        </Dialog.Title>
                ...
                <div class="flex-1 overflow-y-auto px-6 py-5 sm:px-7">
                    <div class="grid gap-4">
                        {#each note.sections as section, index (section.title)}
                            <section class="apple-card-inner p-4">
                                <div class="flex gap-3">
                                    ...
                                    <div class="min-w-0 space-y-2">
                                        <p
                                            class="apple-section-title mb-0 text-[0.74rem]"
                                        >
                                            {section.title}
                                        </p>
                                        <p
