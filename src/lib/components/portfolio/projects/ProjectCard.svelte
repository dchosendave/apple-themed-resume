...
<article
    class="group/project relative flex w-full flex-col gap-3 rounded-[18px] border px-4 py-[15px] text-left transition-[background,border-color,transform,box-shadow] duration-150 [background:var(--ios-chip-bg)] [border-color:var(--ios-glass-border)] hover:-translate-y-px hover:[background:color-mix(in_srgb,var(--ios-chip-bg)_80%,var(--ios-stat-bg))] hover:[border-color:color-mix(in_srgb,var(--ios-blue)_35%,transparent)] hover:shadow-[0_16px_28px_rgba(15,23,42,0.1)] sm:px-[18px] sm:py-4"
>
    <button
    ...
    <div class="relative z-[1] space-y-2">
        ...
        <div class="flex flex-wrap gap-1.5">
            {#if project.signal}
                <span class="apple-badge px-[9px] py-[3px] text-[0.62rem]">
                    {project.signal}
                </span>
            {/if}
            {#if project.role}
                <span class="apple-chip px-[9px] py-[3px] text-[0.62rem]">
                    {project.role}
                </span>
            {/if}
        </div>
    ...
    <p class="relative z-[1] flex-1 text-[0.8rem] leading-[1.58] [color:var(--ios-text-secondary)]">
        {project.description}
    </p>
    ...
    <div class="relative z-[1] mt-auto flex items-center justify-between gap-2">
        ...
        {#if project.url}
            <a
                ...
                class="inline-flex size-7 shrink-0 items-center justify-center rounded-full bg-[var(--ios-blue)] text-[var(--ios-bg)] transition-all duration-200 hover:scale-[1.08] hover:bg-[var(--ios-blue-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ios-blue)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--ios-bg)]"
                title="View project"
    ...
    <span
        class="relative z-[1] mt-auto inline-flex items-center gap-1 text-[0.68rem] font-semibold opacity-[0.72] transition-[opacity,transform] duration-150 group-hover/project:translate-x-0.5 group-hover/project:opacity-100 [color:var(--ios-blue)]"
    >
        Open case file
        <ExternalLinkIcon class="size-3" />
    </span>
